...
//  AsymmetricCrypto
//
//  Created by Viacheslav Zinko on 13/02/19.
//
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var cypheredTextTextfield: UITextView!
    ...
    @IBAction func generateKeyPair(_ sender: AnyObject) {
        ...
        if keyPairExists { // delete current key pair
            AsymmetricCryptoManager.sharedInstance.deleteSecureKeyPair({
				(success) in
                if success {
					...
					self.showAlertWithFadingOutMessage("Error deleting keypair.")
				}
            ...
            AsymmetricCryptoManager.sharedInstance.createSecureKeyPair({
                ...
                if success {
					...
					self.showAlertWithFadingOutMessage("An error happened while generating a keypair: \(String(describing: error))")
				}
                self.view.isUserInteractionEnabled = true
            })
        }
    ...
    @IBAction func cypherText(_ sender: AnyObject) {
        ...
        AsymmetricCryptoManager.sharedInstance.encryptMessageWithPublicKey(clearTextTextfield.text!) { (success, data, error) -> Void in
            if success {
				...
				self.showAlertWithFadingOutMessage("Error cyphering data: \(String(describing: error))")
            }
            self.view.isUserInteractionEnabled = true
        }
    }
    ...
    @IBAction func signText(_ sender: AnyObject) {
        ...
        AsymmetricCryptoManager.sharedInstance.signMessageWithPrivateKey(clearTextTextfield.text!) { (success, data, error) -> Void in
            if success {
				...
				self.showAlertWithFadingOutMessage("Error signing message: \(String(describing: error))")
            }
            self.view.isUserInteractionEnabled = true
        }
    }
    ...
    @IBAction func decypherText(_ sender: AnyObject) {
        ...
        self.clearTextTextfield.text = ""
        self.view.endEditing(true)
        AsymmetricCryptoManager.sharedInstance.decryptMessageWithPrivateKey(encryptedData) {
            ...
            if success {
				...
				self.showAlertWithFadingOutMessage("Error decoding base64 string: \(String(describing: error))")
            }
            self.view.isUserInteractionEnabled = true
        }
    }
    ...
    @IBAction func verifySignature(_ sender: AnyObject) {
        ...
        guard let rawData = clearTextTextfield.text?.data(using: String.Encoding.utf8), let signatureData = Data(base64Encoded: cypheredTextTextfield.text!, options: [])
		else {
        ...
        self.view.isUserInteractionEnabled = false
        self.view.endEditing(true)
        AsymmetricCryptoManager.sharedInstance.verifySignaturePublicKey(rawData, signatureData: signatureData) {
			(success, error) in
